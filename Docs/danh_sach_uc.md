...
**Tiến độ:** 12/26 UC đã đạt (backend + demo UI)
...
- [x] UC-10	Reply to Message	User	User replies to a specific message in a chat.
- [x] UC-11	Add Reaction	User	User adds emoji reaction to a message.
- [x] UC-12	Forward Message	User	User forwards a message to another chat.
...
| UC-10 | Đã đạt | `SendMessage` + `ReplyToMessageId` + quote trong `MessageDto` |
| UC-11 | Đã đạt | `AddReaction` / `RemoveReaction` + hub `MessageReactionUpdated` |
| UC-12 | Đã đạt | `ForwardMessage` (+ hub/REST) |
...
| UC-18, 22 | Schema only | Có field/entity trong DB, chưa có API/handler |
